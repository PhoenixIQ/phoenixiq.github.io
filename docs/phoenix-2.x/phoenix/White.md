...
### 高性能
Phoenix的许多特性都是以提高业务系统的处理性能为目的。通过实现这些技术特性，Phoenix框架不仅能在相同计算资源的情况下，较传统应用架构有巨大的性能提升，此外，在面对大规模并发的场景下，Phoenix框架提供服务的横向扩展能力，支持通过增加服务节点数量提高系统的处理能力。

#### 内存计算(In-Memory)
内存计算将复杂的业务数据模型放到内存中进行查询和修改，提高业务的执行效率。
