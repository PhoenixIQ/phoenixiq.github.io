...
![image](../../../assets/phoenix2.x/phoenix/quick-start/hotel-part-3.png)
...
订单服务也使用了 JPA 存储订单数据，使用可参见：[Spring Data JPA的使用](./popularity-projection#spring-data-jpa的使用)
...
具体参见：[订阅与广播](../../02-phoenix-core/06-subscribe-pub.md)
...
2. 调用所有订单查询接口
```text
GET http://127.0.0.1:8080/order/queryAll/{hotelCode}
```
返回所有订单信息：
```json
[{
...
```text
GET http://127.0.0.1:8080/order/query/{hotelCode}/{orderNumber}
```
返回：
```json
{
